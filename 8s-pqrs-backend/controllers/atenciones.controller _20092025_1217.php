...
switch ($_GET["op"] ?? '') {
    ...
    case 'upsert_cliente_y_atencion':
        ...
        try {
            ...
            if (!is_array($res)) {
                $status = 500;
                if (is_string($res) && stripos($res, 'idCanal no existe') !== false) { $status = 400; }
                http_response_code($status);
            ...
            if ($programarAuto && !empty($res['idAtencion']) && !empty($canalEnvio)) {
                ...
                if (is_array($prog)) {
                    ...
                    // No rompemos el success del upsert, pero reportamos el error de programación
                    $payload['programarError'] = $prog;
                }
            }
    ...
    // === NUEVO: PROGRAMAR DESPUÉS DE UPSERT (SP directo) ===
    case 'programar_auto':
        header('Content-Type: application/json; charset=utf-8');
        $idAtencion = filter_var(param('idAtencion', null), FILTER_VALIDATE_INT);
        ...
        if (is_array($res)) {
            ...
            // Posibles errores del SIGNAL 45000 llegan como texto en $res
            http_response_code(400);
