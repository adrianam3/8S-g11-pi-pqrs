...
function invoke_mapped(object $obj, string $method, array $payload) {
    ...
    try {
        ...
        if (count($params) === 1) {
            // El modelo probablemente espera un array con datos
...
switch ($op) {
    ...
    case 'todos':
        ...
        if (is_array($res) && count($res) > 0) {
            ...
            echo json_encode(["message"=>"No se encontraron PQRS.", "data"=>[]]);
        }
        break;
    ...
    case 'insertar':
        ...
        if (trim((string)$p['asunto']) === '' || trim((string)$p['descripcion']) === '' || !filter_var($p['idTipo'], FILTER_VALIDATE_INT) || !filter_var($p['idCategoria'], FILTER_VALIDATE_INT) || !filter_var($p['idCanal'], FILTER_VALIDATE_INT)) {
        ...
        if (is_numeric($res)) {
            echo json_encode(["success"=>true, "idPqrs"=>(int)$res]);
        } else {
            // Si el modelo devuelve string de error
            $num = filter_var($res, FILTER_VALIDATE_INT);
            if ($num !== false) {
                echo json_encode(["success"=>true, "idPqrs"=>(int)$num]);
            } else {
                http_response_code(500);
    ...
    // === ENDPOINTS AUXILIARES (opcional, solo si tu modelo los tiene)
