...
class EncuestasProgramadas
{
    ...
    public function todos(
        ...
        if (!$stmt) {
            error_log('Error al preparar: ' . $con->error);
            $con->close();
            return false;
        }
    ...
    public function insertar(
        ...
        $canalEnvio = null,
        $enviadoPor = null,
        $observacionEnvio = null,
        $asuntoCache = null,
        $cuerpoHtmlCache = null,
        $tokenEncuesta = null,
        $observaciones = null,
        $estado = 1
    ) {
    ...
    public function activar($idProgEncuesta)
    {
        return $this->cambiarEstado($idProgEncuesta, 1);
    }
    public function desactivar($idProgEncuesta)
    {
        return $this->cambiarEstado($idProgEncuesta, 0);
    }
    ...
    //     return $ok ? 1 : $err;
    // }
    public function sp_registrar_consentimiento($idProgEncuesta, $acepta, $ip, $userAgent, $firmaToken)
    {
        $conObj = new ClaseConectar();
        $con = $conObj->ProcedimientoParaConectar();
        $stmt = $con->prepare('CALL sp_registrar_consentimiento(?, ?, ?, ?, ?)');
        ...
        $stmt->bind_param('iisss', $idProgEncuesta, $acepta, $ip, $userAgent, $firmaToken);
        $ok = $stmt->execute();
        $err = $ok ? null : $stmt->error;
        $stmt->close();
        $con->close();
    ...
    public function clientePorProg(int $idProgEncuesta)
    {
        $conObj = new ClaseConectar();
        $con = $conObj->ProcedimientoParaConectar();

        $sql = "SELECT c.idCliente, c.email, c.nombres, c.apellidos
            FROM encuestasProgramadas ep
    ...
    public function detalleAtencionPorProg(int $idProgEncuesta)
    {
        $conObj = new ClaseConectar();
        $con = $conObj->ProcedimientoParaConectar();

        $sql = "SELECT a.idAtencion, a.detalle, a.fechaAtencion
                FROM encuestasProgramadas ep
