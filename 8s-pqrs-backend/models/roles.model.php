...
class Rol
{
    ...
    public function uno($idRol) // Select * from rol where id = $idRol
    {
        $con = new ClaseConectar();
        $con = $con->ProcedimientoParaConectar();
        $cadena = "SELECT * FROM `roles` WHERE `idRol`=$idRol";
        $datos = mysqli_query($con, $cadena);
        $con->close();
        return $datos;
    }
    ...
    public function insertar($nombreRol, $descripcion, $accesos, $estado) // Insert into rol (...)
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
            $cadena = "INSERT INTO `roles` (`nombreRol`, `descripcion`, `accesos`, `estado`) VALUES ('$nombreRol','$descripcion','$accesos','$estado')";
    ...
    public function actualizar($idRol, $nombreRol, $descripcion, $accesos, $estado) // Update rol set ... where id = $idRol
    {
        try {
            $con = new ClaseConectar();
            $con = $con->ProcedimientoParaConectar();
            $cadena = "UPDATE `roles` SET `nombreRol`='$nombreRol', `descripcion`='$descripcion', `accesos`='$accesos', `estado`='$estado', `fechaModificacion`=CURRENT_TIMESTAMP WHERE `idRol`=$idRol";
            if (mysqli_query($con, $cadena)) {
                return $idRol;
            } else {
                return $con->error;
    ...
    public function eliminar($idRol) // Delete from rol where id = $idRol
    {
        try {
            ...
            $query = "SELECT COUNT(*) as total FROM usuarios WHERE idRol = $idRol";
